...
def hex_to_rgba(h):
    ...
    if len(h) == 6:
        h += "ff"
    return np.array(
        [int(d + u, 16) for d, u in zip(list(h[::2]), list(h[1::2]))]
    )
...
def proj_distance(p: tuple, l: tuple[tuple]):
    ...
    if length == 0:
        return np.sqrt((l1x - p[0]) ** 2 + (l1y - p[1]) ** 2)
    return (
        np.abs((l2y - l1y) * p[0] - (l2x - l1x) * p[1] - l2y * l1x + l2x * l1y)
        / length
    )
...
def rdp(points_list, eps):
    ...
    return (
        np.array(
            [*rdp(points_list[:index], eps), *rdp(points_list[index:], eps)]
        )
        if dmax > eps
        else [points_list[0], points_list[-1]]
    )
...
def rdp_polygon(polygon, eps):
    ...
    return np.concatenate(
        [
            rdp(polygon[: len(polygon) // 2], eps),
            rdp(polygon[len(polygon) // 2 :], eps)[:-1],
        ]
    )
...
def compute_nematic_field(
    ...
    if len(img.data.shape) > 2 and img.data.shape[-1] in [3, 4]:
        ...
        if viewer is not None:
            ...
            instructions.setText(
                "You seem to be using the nematic field computation on an RGB or RGBA image. Please convert to grayscale before using the tool."
            )
    ...
    if decolorize:
        img_data = np.array(
            np.mean(img.data, axis=decolorize_axis), dtype=np.float64
        )  # RGBA channel
    else:
        img_data = img.data
    if normalize_options == "total":
        normalized_img = (img_data - np.min(img_data)) / (
        ...
        normalized_img = img_data
    if len(img_data.shape) > 2:
        nem_field = np.array(
            [
                nf.nematic_field(frame, sigma=sigma, cutoff_ratio=cutoff_ratio)
                for frame in tqdm(
                    normalized_img if normalize else img_data,
                    desc="Computing...",
                    leave=None,
                )
            ]
...
def preview_kernel(**kwargs) -> np.ndarray:  # uses current values in GUI
    ...
    """
    sigma = (
        compute_nematic_field.__signature__.parameters["sigma"].default
        if "sigma" not in kwargs.keys()
        else kwargs["sigma"]
    )
    cutoff_ratio = (
        compute_nematic_field.__signature__.parameters["cutoff_ratio"].default
        if "cutoff_ratio" not in kwargs.keys()
        else kwargs["cutoff_ratio"]
    )
    ...
    if "show" in kwargs.keys():
        if not kwargs["show"]:
            return kernels
    viewer.add_image(
        kernels, name=f"kernels - sigma {sigma} / cutoff_ratio {cutoff_ratio}"
    )
    return kernels
...
def extract_nematic_points_layer(
    ...
    if mask is None:
        mask_data = np.ones(nem_field.shape[:-2])
    else:
        mask_data = mask.data
    if len(mask_data.shape) != len(nem_field.shape[:-2]):
        mask_data = np.array([mask_data for _ in range(nem_field.shape[0])])
    name = img.name
    if len(nem_field.shape) > 4:
        h, w = nem_field.data.shape[1:3]
    else:
        h, w = nem_field.data.shape[:2]  # image shape
    x = y = box_size
    h_overflow, w_overflow = (h % x), (w % y)
    t_crop, b_crop, l_crop, r_crop = 0, -h, 0, -w
    if h_overflow + w_overflow != 0:
        t_crop = h_overflow // 2
        b_crop = h_overflow - t_crop
        l_crop = w_overflow // 2
        r_crop = w_overflow - l_crop
    total_points, total_properties = nf.extract_points(
        nem_field, mask_data, box_size, param_names, invert_mask
    )
    metadata = {
        "shape": nem_field.shape[:-2],
        "box_size": box_size,
        "offset": (t_crop, l_crop),
        "name": img.name,
    }
    points = viewer.add_points(
        np.array(total_points),
        properties=total_properties,
        metadata=metadata,
        size=2,
        name=f"{name} - nematic points layer",
    )
    if return_early:
        return points  # for use in defects
    if points.data.shape[1] < 3:
        points.save(os.path.join(str(folder), f"{name}.csv"))
        del viewer.layers[viewer.layers.index(points)]
        return
    if points.data.shape[1] != 3:
        raise Exception(
            f"Can't handle shape of point data {points.data.shape}."
        )
        return
    if not os.path.isdir(os.path.join(str(folder), name)):
        os.mkdir(os.path.join(str(folder), name))
    time_length = np.max(points.data, axis=0)[0] + 1
    properties_names = list(points.properties.keys())
    for t in tqdm(range(time_length)):
        new_points = (
            points.data[points.data.transpose(1, 0)[0] == t]
            .transpose(1, 0)[1:3]
            .transpose(1, 0)
        )
        new_properties = {}
        for property_name in properties_names:
            new_properties[property_name] = points.properties[property_name][
                points.data.transpose(1, 0)[0] == t
            ]
        ...
        new_layer.save(
            os.path.join(str(folder), name, f"{name} - t{t}.csv")
        )  # save to pathlib folder with its current default name
        del viewer.layers[-1]
    del viewer.layers[viewer.layers.index(points)]
    return True
...
def draw_nematic_field_svg(
    ...
    def _make_lines(frame, t=None):
        boxes = nf.tesselate(
            frame[t_crop:-b_crop, l_crop:-r_crop], box_size, box_size
        )
        nems = []
        ...
        for box, pos in boxes:
            ...
            if lengths:
                p1 = complex(
                    (t1 + t2 * 1j)
                    - 1j * default_length * length_scale * (Qnorm / maxnorm)
                )
                p2 = complex(
                    (t1 + t2 * 1j)
                    + 1j * default_length * length_scale * (Qnorm / maxnorm)
            ...
            if t == None:
                if (
                    ordered != 0
                ):  # meant for 3D view only. testing to see if dynamic slicing is achievable
                    warnings.warn(
                        "You are using the `ordered` parameter, meant for testing magnitude-slicing.\nThis parameter is not meant for practical use at the moment."
                    )
                    if ordered == 1:
                    ...
                    nems.append(
                        np.array(
                            [
                                [order_t, p1.real, p1.imag],
                                [order_t, p2.real, p2.imag],
                            ]
                        )
                    )
                else:
                    nems.append(
                        np.array([[p1.real, p1.imag], [p2.real, p2.imag]])
                    )
            else:
                nems.append(
                    np.array([[t, p1.real, p1.imag], [t, p2.real, p2.imag]])
                )
    ...
    if len(nem_field.shape) > 4:
        ...
        maxnorm = np.max(
            [
                [
                    nf.extract(np.mean(box, (0, 1)))[0]
                    for box, pos in nf.tesselate(
                        frame[t_crop:-b_crop, l_crop:-r_crop],
                        box_size,
                        box_size,
                    )
                ]
                for frame in nem_field
            ]
        )
        total_properties = {"Qxx": [], "Qxy": [], "norm": [], "angle": []}
        for t, frame in enumerate(nem_field):
            nems, properties = _make_lines(frame, t)
            for p in total_properties.keys():
                total_properties[p] += properties[p]
            total_nems += nems
    else:
        h, w = nem_field.shape[:2]
        t_crop, b_crop, l_crop, r_crop = _get_crop(h, w)
        boxes = nf.tesselate(
            nem_field[t_crop:-b_crop, l_crop:-r_crop], box_size, box_size
        )
        maxnorm = np.max(
            [nf.extract(np.mean(box, (0, 1)))[0] for box, pos in boxes]
        )
    ...
    if custom_kwargs != "":
        for argval in re.split("-", custom_kwargs):
            ...
            if arg in ["color", "colormap", "width", "contrast_limits"]:
                arg = "edge_" + arg
            if arg in ["opacity", "edge_width", "z_index"]:
                val = float(val)
            if arg == "edge_color":
                val = _process_color(val)
            if arg == "edge_contrast_limits":
                val = [float(v) for v in re.split(",", val[1:-1])]
            kwargs[arg] = val
    return viewer.add_shapes(
        total_nems,
        properties=total_properties,
        name=f"{name} - nematic field",
        **kwargs,
    )
...
def cluster_defects(
    ...
    def _find_og_position(p0, p1):
        return (
            p0 * box_size + t_crop + box_size // 2,
            p1 * box_size + l_crop + box_size // 2,
        )
    ...
    def maxconvolution(img):
        ...
        for p0, p1 in nf.loop_over_positions(img.shape[:2]):
            sub = img[
                max(0, p0 - m) : min(p0 + m, img.shape[0]) + 1,
                max(0, p1 - m) : min(p1 + m, img.shape[1]) + 1,
            ]
            rgba = np.array(
                [
                    np.max(sub[:, :, 0]),
    ...
    # add 1 for padding
    points_list_normalised = [
        (point[0] // box_size + 1, point[1] // box_size + 1)
    ...
    for k in range(1, np.max(clusters) + 1):
        cluster = (
            clusters == k
        ) * offset  # arbitrary negative offset for edge detection and debugging
        # edge detection accounts for box padding
        if np.any(cluster[1 :: (h // box_size - 1)] == offset) or np.any(
            cluster[:, 1 :: (w // box_size - 1)] == offset
        ):
            edge_props.append(
                np.array(
                    [
                        _find_og_position(*(p - 1))
                        for p in measure.find_contours(
                            cluster,
        ...
        for e, p in enumerate(
            measure.find_contours(
                cluster, positive_orientation="low", fully_connected="low"
            )[0]
        ):
            ...
            p0high, p1high = int(p[0] - 0.5), int(p[1] - 0.5)
            ...
            if [r, c] not in points_list:
                break
            angles.append(
                points.properties["angle"][points_list.index([r, c])]
            )
            ...
            for target, color, charge in zip(
    ...
    for p0, p1 in nf.loop_over_positions((h // box_size, w // box_size)):
        ...
        clusters_img[
            r - box_size // 2 : r + (box_size + 1) // 2,
            c - box_size // 2 : c + (box_size + 1) // 2,
        ] = (
            np.zeros(4)
            if clusters[p0 + 1, p1 + 1] - 1 < 0
            else hex_to_rgba(properties["color"][clusters[p0 + 1, p1 + 1] - 1])
        )
        edges_img[
            r - box_size // 2 : r + (box_size + 1) // 2,
            c - box_size // 2 : c + (box_size + 1) // 2,
        ] = (
            np.ones(4) * (img_edges_small[p0 + 1, p1 + 1] > 0) * 255
        )  # hex_to_rgba here also ?

    images = []
    if mode == "squares":
        images.append(
            viewer.add_image(
                np.array(clusters_img, dtype=np.uint8),
                blending="translucent",
                opacity=1,
                name=f"{name} - clusters",
            )
        )
        images.append(
            viewer.add_image(
                np.array(edges_img, dtype=np.uint8),
                blending="translucent",
                opacity=1,
                name=f"{name} - edge clusters",
            )
        )
    ...
    edge_props = [
        ...
        for j in [
            k
            for k, prop in enumerate(
                [p if len(p) > 3 else None for p in edge_props]
            )
            if prop is not None
        ]
    ]
    ...
    clusters_shapes = viewer.add_shapes(
        [
            rdp_polygon(prop[:-1], rdp_eps) for prop in props
        ],  # replace props by props[:3] and it does work, somehow
        shape_type="polygon",
        face_color=properties["color"],
    ...
    edge_clusters_shapes = viewer.add_shapes(
        [rdp_polygon(prop[:-1], rdp_eps) for prop in edge_props],
        shape_type="polygon",
        opacity=1,
        edge_width=0,
        name=f"{name} - edge clusters",
...
def box_intersection_defects(points: Points):
    ...
    for idx, (r, c) in enumerate(points_list):
        if np.all(
            [neighbour in points_list for neighbour in neighbours(r, c)]
        ):
            idc = [idx] + [
                points_list.index(neighbour) for neighbour in neighbours(r, c)
            ]
            ...
            if not reached_target:
                defect_charge, defect_color = "N/A", "#ff33ff"
                raise BaseException(
                    "Computation error, contact the developers with steps to reproduce."
                )
            properties["value"].append(defect_charge)
            properties["color"].append(defect_color)
...
def find_defects(
    ...
    if box_size == -1:
        box_size = draw_nematic_field_svg.__signature__.parameters[
            "box_size"
        ].default
    points = extract_nematic_points_layer(
        img, box_size=box_size, return_early=True
    )
    if len(img.data.shape) == 3:
        all_points, all_properties, metadata = (
            points.data,
            points.properties,
            points.metadata,
        )
        ...
        for t in np.unique(all_points[:, 0]):
            ...
            properties = {
                k: v[nb_of_points_seen : nb_of_points_seen + len(points_list)]
                for k, v in all_properties.items()
            }
            nb_of_points_seen += len(points_list)
            points = viewer.add_points(
                points_list, properties=properties, metadata=metadata
            )
            clusters, edge_clusters, images = cluster_defects(
                points, thresh, mode=mode
            )
            if mode == "squares":
            ...
            del viewer.layers[viewer.layers.index(points)]
            all_clusters_data += [
                np.c_[t * np.ones(len(shape)), shape]
                for shape in clusters.data
            ]
            all_edge_clusters_data + [
                np.c_[t * np.ones(len(shape)), shape]
                for shape in edge_clusters.data
            ]
        ...
        both = viewer.add_shapes(
            list(all_clusters_data) + list(all_edge_clusters_data),
            shape_type="polygon",
            name=f"{img.name} temp defects layer",
        )
        points = extract_nematic_points_layer(
            img,
            mask=1 * (both.to_labels(img.data.shape) == 0),
            box_size=box_size,
            return_early=True,
        )
        all_box_defects = []
        ...
        for t in np.unique(all_points[:, 0]):
            ...
            properties = {
                k: v[nb_of_points_seen : nb_of_points_seen + len(data_t)]
                for k, v in all_properties.items()
            }
            box_defects, box_properties = box_intersection_defects(
                points_t := viewer.add_points(
                    data_t, properties=properties, metadata=points.metadata
                )
            )
            all_box_defects += [
                np.array([[t, *d] for d in defect]) for defect in box_defects
            ]
        ...
        if mode == "simplified":
            ...
            clusters.add_ellipses(
                all_box_defects, face_color=all_box_properties["color"]
            )
            N = len(all_box_properties["value"])
            if len(clusters.properties["value"]) == 0:
                clusters.properties = {
                    "value": all_box_properties["value"],
                    "color": all_box_properties["color"],
                }
            elif N > 0:
            ...
            clusters_img = viewer.add_image(
                np.array(all_images["clusters"], dtype=np.uint8),
                blending="translucent",
                opacity=1,
                name=f"{img.name} - clusters",
            )
            edge_clusters_img = viewer.add_image(
                np.array(all_images["edge_clusters"], dtype=np.uint8),
                blending="translucent",
                opacity=1,
                name=f"{img.name} - edge clusters",
            )
        ...
        both = viewer.add_shapes(
            list(clusters.data) + list(edge_clusters.data),
            shape_type="polygon",
            name=f"{img.name} temp defects layer",
        )
        ...
        if mode == "squares":
            ...
            clusters.add_ellipses(
                box_defects, face_color=box_properties["color"]
            )
            N = len(box_properties["value"])
            if len(clusters.properties["value"]) == 0:
                clusters.properties = {
                    "value": box_properties["value"],
...
def get_help(widget: str):
    if widget not in tooltips.keys():
...
for function in functions:
    function._widget._mgui_set_tooltip(tooltips[function.name])
