...
def make_random_line_image(
    h: int, w: int, n: int
) -> np.ndarray:  # eventually delete this function
    ...
    for r0, c0 in zip(np.random.randint(0, h, n), np.random.randint(0, w, n)):
        dr, dc = np.random.randint(
            max(-int(h / 10), -int(w / 10)), min(int(h / 10), int(w / 10)), 2
        )
        r1, c1 = r0 + dr, c0 + dc
        if (r0 + dr >= h or c0 + dc >= w) or (r0 + dr < 0 or r0 + dc < 0):
            continue
...
def tesselate(img: np.ndarray, x: int, y: int) -> list[np.ndarray]:
    ...
    if x * th != h or y * tw != w:
        raise Exception("Dimensions provided aren't divisors of image shape.")
    return [
        (img[dh : dh + x, dw : dw + y], (dh, dh + x, dw, dw + y))
        for dw in range(0, w, y)
        for dh in range(0, h, x)
    ]
...
def kernel(dr: np.ndarray, sigma: float) -> np.ndarray:
    ...
    return np.exp(-(np.linalg.norm(dr) ** 2) / (2 * sigma**2)) * np.array(
        [
...
def kernel_xx(dr: np.ndarray, sigma: float) -> float:  # internal
    ...
    phi += np.pi * (phi < 0)  # angle in [0,pi)
    return (
        np.exp(-(sum(dr**2) ** 2) / (2 * sigma**2)) * np.cos(2 * phi) / sigma
    )
...
def kernel_xy(dr: np.ndarray, sigma: float) -> float:  # internal
    ...
    phi += np.pi * (phi < 0)  # angle in [0,pi)
    return (
        np.exp(-(sum(dr**2) ** 2) / (2 * sigma**2)) * np.sin(2 * phi) / sigma
    )
...
def kernels(sigma: float, cutoff_ratio: float) -> tuple[np.ndarray]:
    ...
    if radius < 1:
        raise Exception(
            "Please choose a bandwidth and cutoff_ratio such that the kernel radius is greater or equal to 3."
        )
    Kxx, Kxy = np.zeros((2 * radius + 1, 2 * radius + 1)), np.zeros(
        (2 * radius + 1, 2 * radius + 1)
    )
    # radius*np.linspace(-1,1,2*radius+1)
    center = np.array((radius, radius))
    for p in loop_over_positions(Kxx.shape):
        if sum(np.array(p - center) ** 2) > radius**2:
            continue
        Kxx[p], Kxy[p] = kernel_xx(p - center, sigma), kernel_xy(
            p - center, sigma
        )
    return Kxx, Kxy
...
def nematic_field(
    a: np.ndarray, sigma: float, cutoff_ratio: float
) -> np.ndarray:
    ...
    Qxx, Qxy = (
        a
        * convolve(a - np.mean(a), Kxx, mode="full")[
            radius:-radius, radius:-radius
        ],
        a
        * convolve(a - np.mean(a), Kxy, mode="full")[
            radius:-radius, radius:-radius
        ],
    )
...
def draw_nematic_field(
    nem_field: np.ndarray, box_size: int, **kwargs
) -> np.ndarray:
    ...
    """
    uniform_bg = (
        True if "uniform_bg" not in kwargs.keys() else kwargs["uniform_bg"]
    )
    ...
    if h_overflow + w_overflow != 0:
        ...
        if r_crop == 0:
            r_crop = -w
    imghelper = np.zeros(
        (h - h_overflow, w - w_overflow)
    )  # of size divisible by x,y,nx,ny ? normally
    h, w = imghelper.shape  # cropped shape

    if lengths:
        rl, cl = max((min(h / nx, w / ny) // 8), 1), max(
            (min(h / nx, w / ny) // 2), 1
        )
        if rl == 1 or cl == 1:
            ...
            def drawer(
                t1, t2, Qnorm
            ):  # multiply by 1j because rows are imaginary part and columns are real part
                left, right = complex(
                    (t1 + t2 * 1j) - 1j * (min(x, y) // 4) * np.exp(1j * phi)
                ), complex(
                    (t1 + t2 * 1j) + 1j * (min(x, y) // 4) * np.exp(1j * phi)
                )
                return sk.draw.line(
            ...
            def drawer(t1, t2, Qnorm):
                return sk.draw.ellipse(
                    t1, t2, rl, max(cl * Qnorm, 1), (h, w), rotation=phi
                )
    ...
    maxnorm = (
        np.max([extract(np.mean(box, (0, 1)))[0] for box, pos in boxes]) / 2
        if lengths
        else 1
    )
...
def extract_points(
    ...
    def _make_points_for_frame(nem_frame, mask_frame):
        boxes = tesselate(
            nem_frame[t_crop:-b_crop, l_crop:-r_crop], box_size, box_size
        )
        ...
        for box, pos in boxes:
            ...
            if ((mask_frame[t1, t2] > 0) and not invert_mask) or (
                not (mask_frame[t1, t2] > 0) and invert_mask
            ):
                points.append([t1, t2])
                if "Qxx" in params:
                    properties["Qxx"].append(Qxx)
                if "Qxy" in params:
                    properties["Qxy"].append(Qxy)
                if "norm" in params:
                    properties["norm"].append(norm)
                if "angle" in params:
                    properties["angle"].append(angle)
    ...
    if len(nem_field.shape) > 4:
        ...
        for t in _tqdm(
            range(len(nem_field)), desc="Generating Points...", leave=None
        ):
            points, properties = _make_points_for_frame(
                nem_field[t], np.unstack(mask[slice(t, t + 1)])[0]
            )
            total_points += [[t] + p for p in points]
            for param_name in params:
                total_properties[param_name] += properties[param_name]
    else:
        total_points, total_properties = _make_points_for_frame(
            nem_field, mask
        )
